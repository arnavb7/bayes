...
def fillObsCPT(bayesNet, gameState):
    ...
    obsPositionToHouseVals = {}
    for housePos in gameState.getPossibleHouses():
        houseVal = posToVal[housePos]
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            if obsVar not in obsPositionToHouseVals:
                obsPositionToHouseVals[obsVar] = set()
            obsPositionToHouseVals[obsVar].add(houseVal)

    for obsVar, adjacentHouseVals in obsPositionToHouseVals.items():
        ...
        for assignment in obsFactor.getAllPossibleAssignmentDicts():
            ...
            foodHere = foodHouseVal in adjacentHouseVals
            ghostHere = ghostHouseVal in adjacentHouseVals

            if not foodHere and not ghostHere:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    marginal = inference.inferenceByVariableElimination(bayesNet, [FOOD_HOUSE_VAR], evidence, eliminationOrder)

    bestProb = -1
    bestAssignment = None
    for assignment in marginal.getAllPossibleAssignmentDicts():
        p = marginal.getProbability(assignment)
        if p > bestProb:
            bestProb = p
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        joint = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)

        assignLeft = dict(evidence)
        assignLeft[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        assignLeft[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL
        pFoodLeft = joint.getProbability(assignLeft)

        assignRight = dict(evidence)
        assignRight[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        assignRight[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        pFoodRight = joint.getProbability(assignRight)

        leftExpectedValue = pFoodLeft * WON_GAME_REWARD + pFoodRight * GHOST_COLLISION_REWARD
        rightExpectedValue = pFoodRight * WON_GAME_REWARD + pFoodLeft * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, outcome in self.getExplorationProbsAndOutcomes(evidence):
            leftVal, rightVal = self.computeEnterValues(outcome, enterEliminationOrder)
